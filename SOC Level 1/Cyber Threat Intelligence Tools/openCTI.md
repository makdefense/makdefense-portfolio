...
**Lab Topic**: [OpenCTI]  
**Date Completed**: [10/25/2025]

---

## 🧠 Summary

> In this lab, I learned about the importance of understanding OpenCTI and how it is used, which is that OpenCTI is an open-source Threat Intelligence Platform (TIP) that centralizes, structures,
and operationalizes CTI (STIX/TAXII, sightings, feeds, reports) so your SOC/IR/CTI teams can enrich alerts, hunt for actor/TTP context, automate sharing, and feed detections - all from a single knowledge
graph. Next, I learned about the importance of understanding how to navigate through the platform, which is that knowing how to move fast and confidently in OpenCTI turns raw threat data into real
decisions - faster triage, clearer context, better hunts, and reliable automation. Navigation skills let you get the correct intel to the right playbook at the right time. Finally, I learned about the
importance of understanding what functionalities will be necessary during a security threat analysis, which is that knowing which platform functionalities matter - and how to use them - is what turns raw
alerts into accurate, fast, and defensible decisions. Those capabilities let you detect, contain, and learn from attacks rather than react to noise.
...
> After reading the section "OpenCTI Dashboard 1," I was tasked with a mini-practical. For the first question, I had to figure out the group that uses the 4H RAT malware. To figure this out
I first launched Mozilla Firefox within the THM AttackBox and navigated to the website "http://10.201.99.184:8080/", which displayed the OpenCTI login screen.

> <img width="735" height="728" alt="opencti" src="https://github.com/user-attachments/assets/e215285b-cd72-43cf-8046-5532c15dc6f6" />

> After using the login credentials "info@tryhack.io" and "TryHackMe1234" to log in to the dashboard, I then navigated to the left and clicked "Arsenal," then saw that the group "Putter Panda" uses the
"4H RAT" malware.
...
> Moving on to the next question, I had to figure out which kill-chain phase is linked to the "Command-Line Interface" Attack Pattern. To figure this out, I simply navigated to the top of the application,
...
> For the last question, I had to figure out which tab would house the indicators. To figure this out, I simply navigated around the application and saw that the tab
...
> Moving on to the next mini practical for the first question, I had to figure out which intrusion sets are associated with the Cobalt Strike malware with a Good confidence level. To figure this out, I
first navigated to the "Arsenal" tab, searched "Cobalt Strike," and selected it.
...
> I then scrolled down to the "Last Used Relationships" section and selected the first Intrusion Set labelled "Good," which was "FIN7."
...
> For this practical, I had to play the role of a SOC analyst, investigate both the "CaddyWiper" malware and the "APT37" group using OpenCTI, and answer questions. For the first question, I had to figure
out the earliest date recorded related to CaddyWiper. To figure this out, I navigated to the left of the dashboard, selected "Arsenal," searched for "CaddyWiper," and selected it.
...
> I then scrolled down to "LATEST REPORTS ABOUT THIS ACTIVITY," selected "Ciso CaddyWiper," and saw that the earliest date was 2022/03/15
...
> Next question I had to figure out how many malware relations were linked to this Attack Technique. To figure this out, I selected "Native API," navigated to the top of the dashboard, selected
"knowledge," and saw under "DISTRIBUTION OF RELATIONS" the total number of malware relations was 113.

> <img width="1434" height="531" alt="cti18" src="https://github.com/user-attachments/assets/22a0e9f0-8412-4919-9628-b091e9d49a6d" />

> For the next question, I had to identify the three tools used by the Attack Technique "Native API" in 2016. To figure this out, I navigated to the right of the dashboard, selected "Tools,"
then selected "Knowledge," which retrieved the three tools: "BloodHound, Empire, ShimRatReporter."
...
> For the next question, I had to figure out which country APT37 was associated with. To figure this out, I navigated to the left side of the dashboard, selected "Arsenal," searched "APT37," selected
...
> For the final question, I had to figure out which Attack Techniques the Native API uses for initial access. To figure this out, I navigated to the top of the dashboard, selected the "Knowledge" tab,
scrolled down to where it said "initial-access" and saw the two techniques: "T1189" and "T1566."
