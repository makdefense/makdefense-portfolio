...
> In this lab, I learned the importance of understanding common Initial Access methods on Windows. This knowledge helps defenders recognize how attackers first enter systems and
stop them early. Detecting signs like suspicious logins, malicious files, or unusual activity prevents attackers from escalating privileges or causing damage. These skills build
essential SOC analyst abilities, strengthen incident response, and improve an organization’s overall security posture.
...
For the first question, I had to determine which user appeared to be the most actively brute-forced by botnets. To figure this out, I launched the RDP-Security.evtx file and 
navigated to the event with the timestamp 5/20/2025 9:57:30 AM and Event ID 4625. Upon further investigation, I determined that the user most actively brute-forced by botnets was 
"Administrator."
>
...
> Moving on to the next question, I had to identify the real workstation name (hostname) of the threat actor. To determine this, I navigated to the Network Information section for
Event ID 4624 and saw that the real workstation name of the threat actor was "DESKTOP-QNBC4UU."
>
...
For the first question, I had to find a hidden flag by running the "www,skype.com" file from the Phishing Case 1 folder. After launching the file, I retrieved the hidden flag:
...
> For the next question, I had to determine which URL the malicious LNK file used to download the next-stage malware by investigating the second attachment from the Phishing Case 2
folder. After navigating to and selecting the "Official Website" shortcut, I right-clicked it and selected Properties. From there, I retrieved the URL:
...
> For the last question of this section, I had to find the name of the double-extension file within the Phishing Case 3 folder. After opening the folder, I identified the file:
best-cat.jpg.exe
>
...
For the first question, I had to determine which file the user downloaded via the web browser. I launched the log file and selected Event ID 11 with the timestamp 5/20/2025 6:58:28 
PM. Upon further investigation, I discovered that the downloaded file was:
...
> For the next question, I had to determine the folder the user used to extract the suspicious file. I reviewed Event ID 11 with the timestamp 5/20/2025 6:58:43 PM and identified
the extraction folder as:
C:\Users\Administrator\Pictures
>
> <img width="1300" height="378" alt="18" src="https://github.com/user-attachments/assets/95398ae8-bf74-4992-8f94-57986f86d271" />

> For the second-to-last question, I had to determine the process ID of the launched phishing malware. I selected Event ID 22 with the timestamp 5/20/2025 6:59:18 PM and discovered
that the process ID was 5484. I also identified that the malicious domain the malware attempted to connect to was:
rjj.store
>
...
For the first question, I had to determine which USB file was launched by the user. I opened the log file and selected Event ID 1 with the timestamp 5/20/2025 7:34:06 PM, which 
showed that the file launched by the user was:
...
> For the next question, I had to determine which malicious file the malware dropped to disk. I selected Event ID 13 with the timestamp 5/20/2025 7:34:06 PM and discovered that the
file dropped was:
...
> For the final question, I had to determine which other USB drive the malware propagated to. I selected Event ID 11 with the timestamp 5/20/2025 7:34:06 PM and discovered that the
malware propagated to:
*F:*
>
