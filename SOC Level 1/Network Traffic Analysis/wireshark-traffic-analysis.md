...
> In this lab, I learned the importance of performing traffic analysis with Wireshark and identifying anomalies within a network. Wireshark traffic analysis is critical because
nearly every cyberattack leaves behind network evidence. By understanding what normal traffic looks like, analysts can quickly detect anomalies such as malware beaconing, data
exfiltration, port scans, or suspicious foreign connections. This skill is essential for SOC analysts, helps validate SIEM alerts, and distinguishes true cybersecurity
professionals from tool-dependent beginners. Mastering Wireshark provides a significant advantage in investigations, certifications, and job interviews.
...
> For this room, I had to investigate packet-level details by synthesizing the analyst's knowledge and Wireshark functionality for detecting anomalies and odd situations for a
given case. For the first section of the room, I had to answer a few questions within the "Nmap Scans" section by investigating an "Exercise.pcapng" file.
...
> For the first question, I had to find the total number of "TCP Connect" scans. To figure this out after launching the "Exercise.pcapng" file with Wireshark, I navigated to the
...
I pressed "Enter," which then showed the total number of "TCP Connect" scans as "1000."
...
> Moving on to the next question in this section, I had to figure out how many "UDP close port" messages there were. To figure this out, I navigated back to the top of the
Wireshark web application's input field, input "udp," then "enter" to filter out all udp packets. I then selected the 4 closed packets, then navigated to the bottom left,
...
> Moving on to the last question for this section, I had to figure out which UDP port in the 55-70 port range was open. To figure this out, I navigated back to the top of the
...
of ARP requests crafted by the attacker. To figure this out, I had to first launch the "Exercise.pcapng" file, then select the first ARP packet, and then navigate to the bottom-
left under the ARP dropdown menu, select "Opcode: request (1)" and drag it to the input field above. This then filtered out all the ARP packets within the "Exercise.pcapng" file.
...
> Moving on to the next question, I had to determine the number of HTTP packets the attacker received. To figure this out, I navigated back to the top of the input field,
...
there were a total of 6 sniffed username&password entries.
...
> Moving on to the last question, I had to figure out the comment from "Client354." To figure this out, I navigated to the top of the input field, inputted the filter
expression "http contains 'client354'", then "enter", which retrieved the packet. After retrieving the packet, I then navigated to the bottom-left, clicked the drop-down
...
> For the next question, I had to figure out how many NETBIOS registration requests the "LIVALJM" workstation has. To figure this out I navigated to the top of the Wireshark
...
I retrieved 16 registration requests.
...
> Moving on to the next question, I had to figure out which host requested the IP address "172.16.13.85." To figure this out, I navigated to the top of the Wireshark application,
input "dhcp," then pressed "enter." After all the DHCP packets were loaded, I navigated to packet 18413, which was a DHCP request. After selecting the specific packet
...
"(dhcp ) && (dhcp.option.requested_ip_address == 172.16.13.85)." I then pressed "enter," which retrieved packet number 72529. I then selected the packet, navigated to the
bottom left, and clicked on the drop-down menu "Option: 12" to retrieve the hostname "Galaxy-A12."
...
I had to find the IP address for user "u5" and enter it in defanged format. To figure this out, I first selected a packet within the "pcap" file, then navigated to the
...
which then retrieved the user "u5." I then selected a packet that contained the user "u5," navigated back to the bottom left, selected the Internet Protocol Version 4 drop-down
menu, which also had the IP address of the "u5" user. I was asked to enter it in defanged format, so the defanged IP address turned out to be "10[.]1[.]12[.]2."
...
> Moving on to the last question in this section, I had to determine the hostname of the available host from the Kerberos packets. To figure this out, I simply scrolled down to
packet number 8 within the pcap file and saw that the available hostname was "xp1$."
...
> For this section, I had to investigate the anomalous packets in the pcap file "icmp-tunnel.pcap," located in the directory "Desktop/exercise-pcaps/dns-icmp/."
For the first question, I had to figure out which protocol is used in ICMP tunneling. To figure this out, I simply scrolled to packet number 42, selected it, then navigated to
...
> Moving on to the next question, I had to investigate the anomalous packets in the pcap file "dns.pcap," located in the directory "Desktop/exercise-pcaps/dns-icmp/."
...
total number of incorrect login attempts is 737.
...
19770 and saw the resume.doc file size to be 39424.
...
> For the last question within this section, I had to figure out the command used by the adversary when they tried to assign special flags to change the executing permissions of
the uploaded file. To figure this out, I just scrolled through the filtered packets and noticed that packet 19832 contained the command used by the adversary: "CHMOD 777."
...
file, navigated down to the bottom-left, selected the "user-agent" from the "HTTP" drop-down menu, right-clicked it, then selected "Apply as Column." After applying the filter, I
navigated through all the "user-agents" and found a total of 6 different anomalous "user-agent" types.
...
6 different user-agent types, and saw the packet number was 52.

> <img width="1031" height="149" alt="76" src="https://github.com/user-attachments/assets/ed0d5362-0cdb-4e74-8789-8117e553339f" />

> For the next question, I had to determine the packet number for the "Log4j" attack's initial phase. To figure this out, I had to first launch the
"~/Desktop/exercise-pcaps/http/http.pcapng" file, then navigated to the "User-Agent," selected it, then right-clicked, selected "Remove This Column." After removing the "User-
Agent" column I navigated to the top of the input field, entered "http contains 'jndi'", and saw that packet number 444 had the attack starting phase for "Log4j" with the
"POST" request.
...
then "Value." I then launched CyberChef, selected the recipe "From Base 64," pasted the value I copied from Wireshark, and the IP address 62.210.130.250 appeared in the output. So, in defanged format, the IP address would be "62[.]210[.]130[.]250."
...
I navigated to the bottom-left, selected the "Handshake Protocol" drop-down menu, then expanded the "Server Name indication extension," selected the full "Server Name:...,"
dragged it to the input field above, changed the server name from "clientservices.googleapis.com" to "accounts.google.com," which gave me the correct packet number 16.
...
> Moving on to the second question, I had to determine the number of HTTP2 packets by decrypting the traffic using the "KeysLogFile.txt" file. To figure this out, I navigated to
the top of the Wireshark application, selected "Edit," then "Preferences," then scrolled down to "TLS," selected that, then clicked on the "Browse..." button under
...
adding the file, I clicked the "OK" button at the bottom, then navigated to the top of the Wireshark application, entered "http2" within the input field, and pressed the "Enter"
button. After doing all of this, I found the total number of HTTP2 packets to be "115."
...
> Moving on to the third question, I had to determine the authority header of the HTTP/2 packet for frame 322 and enter the address in defanged format. To figure this out I simply
...
> Moving on to the last question in this section, I had to investigate the decrypted packets to find a hidden flag. To do this, I navigated to the top of the Wireshark
application, clicked on the small search icon, selected “Narrow (UTF-8 / ASCII),” and then entered “flag{” into the String input field before clicking the Find button.
After completing this step, I was able to retrieve the flag “FLAG{THM-PACKETMASTER},” which was located under the “Line-based text data: text/plain (38 lines)” drop-down menu.
...
> For this section, I had to investigate the “Desktop/exercise-pcaps/bonus/Bonus-exercise.pcap” file to answer the following questions. For the first question, I needed to
identify the packet number that contained credentials using HTTP Basic Authentication.
To do this, I first opened the “Desktop/exercise-pcaps/bonus/Bonus-exercise.pcap” file from the Desktop. I then navigated to the top of the Wireshark application, selected Tools,
and clicked Credentials. After scrolling through the results, I observed that packet number 237 was using HTTP Basic Authentication credentials.
...
> Moving on to the second question, I had to determine the packet number where an empty password was submitted. To do this, I first navigated to packet number 41 because it
contained the request “PASS: …”. I then moved to the bottom-left panel, expanded the File Transfer Protocol (FTP) drop-down menu, selected Request command: PASS, and dragged it
into the top filter input field. This action returned packet number 170, which appeared to contain the empty password request.
...
> For the last section of the room, I had to investigate the “Desktop/exercise-pcaps/bonus/Bonus-exercise.pcap” file. For the first question, I needed to determine the rule for
denying a source IPv4 address by creating a rule based on packet number 99.
To do this, I selected packet number 99, navigated to the top of Wireshark, clicked Tools, then Firewall ACL Rules, and selected IPFirewall (ipfw) for the “Create rules for”
option. Wireshark then generated the rule for denying the source IPv4 address: add deny ip from 10.121.70.151 to any in
...
> For the last question, I had to determine the rule for allowing a destination MAC address by creating a rule based on packet number 231. To do this, I first selected packet
number 231, then navigated to the top of Wireshark, clicked Tools, and selected Firewall ACL Rules. I then chose IPFirewall (ipfw) for the “Create rules for” option and deselected
the Deny checkbox. After doing this, I observed that the rule for allowing the destination MAC address was: add allow MAC 00:d0:59:aa:af:80 any in
...
> This lab provided me with foundational knowledge of network traffic analysis using Wireshark and taught me how to identify anomalies within network traffic.
