...
> In this lab, I learned about the importance of understanding the fundamentals of using Wireshark, which teaches you how networks actually communicate, not just in theory.
It allows you to analyze real traffic, investigate attacks, validate alerts, and spot malicious behavior that logs can miss. For SOC and blue-team roles, packet analysis with
Wireshark is a core skill that proves you can investigate incidents, not just read dashboards.
...
After launching the THM AttackBox, I double-clicked the "Exercise.pcapng" file to open it in Wireshark and start the exercise.

> <img width="503" height="370" alt="1" src="https://github.com/user-attachments/assets/86c7e3b1-177e-4c3b-889c-89bb50393252" />

*** Statistics | Summary ***

> For the first exercise question in this section, I had to determine the IP address of the hostname that starts with "bbc." To figure this out, I had to investigate the
...
"bbc," which then retrieved the IP address of "199.232.24.81" for the hostname that starts with "bbc."
>
...
> For the next question, I had to figure out how many bytes (k) were transferred from the "Micro-ST" MAC Address. To figure this out, I navigated to the top of Wireshark
...
> For the next question, I had to determine the number of "Kansas City"- linked IP addresses. To figure this out, I navigated to the top of the Wireshark application, clicked.
...
> For the last question of this section, I had to figure out which IP address was linked with the "Blicnet" AS Organisation. To figure this out, I navigated to the top of the
Wireshark application, clicked "Endpoints," then navigated to the "AS Organisation" sub-header within the "IPv4" section, scrolled to find "Blicnet." After finding the
organization, I retrieved the IP address to be "188.246.82.7" with "Blicnet" AS Organisation.
>
...
> Moving on to the next section of this lab, for the first question of this exercise, I had to figure out the most used IPv4 destination address. To figure this out, I navigated
to the top of the Wireshark application, clicked "Statistics" header, then "Source and Destination Addresses," then clicked "Destination IPv4 Addresses" dropdown menu to expand
it, then I hovered over the "Count" dropdown menu, clicked it, which then retrieved the most used IPv4 address, "10.100.1.33."
>
...
> For the next question, I had to determine the maximum service request-response time for DNS packets. To figure this out, I navigated to the top of the Wireshark application,
...
> For the last question of this exercise, I had to determine the total number of HTTP requests made to "rad[.]msn[.]com." To figure this out, I navigated to the top of the
Wireshark application, clicked "Statistics," then "HTTP," then "Requests," then under "HTTP Requests by HTTP HOST," I selected the domain "rad.msn.com," scrolled right to see
that the total number of HTTP Requests accomplished was "39."
>
...
> Moving on to the next section of this lab, for the first question of this exercise, I had to figure out the total number of IP packets. To figure this out, I navigated to the
"Current filter" search bar, input "IP," then navigated to the right, and clicked the green arrow. After doing this, I retrieved the total number of IP packets to be
"81420."
>
...
> For the next question, I had to figure out how many packets had a "TTL value less than 10." To figure this out, I navigated to the "Current filter" search bar, inputted
...
> For the next question, I had to figure out the number of packets that use "TCP port 4444." To figure this out, I navigated to the "Current filter" search bar and input
...
> For the next question, I had to figure out the number of "HTTP GET" requests sent to port 80. To figure this out, I navigated to the "Current filter" search bar and input
"http.request.method == "GET" && tcp.port == 80," then navigated to the right, and clicked the green arrow. After doing this, I retrieved the total number of requests to be
"527."
>
...
> Moving on to the last section of the lab for the first question, I had to find all Microsoft IIS servers, then figure out the number of packets that did not originate from
...
After doing this, I retrieved the total number of packets as "21."
>
...
> For the next question, I had to figure out how many packets have version "7.5." To figure this out, I navigated to the search bar, inputted
...
> For the next question, I had to determine the total number of packets using ports 3333, 4444, or 9999. To figure this out, I navigated to the top of the search bar,
...
navigated to the top of the application, selected "Analyze," then "Display Filter Expression," then selected "tcp.checksum.status . Checksum Status," then "Bad" for the
...
> For the last question in this section, I had to determine the number of displayed packets using the same existing filtering button to filter traffic. To figure this out,
I navigated to the right, clicked "gif/jpeg with http-200," then navigated to the right and clicked the green arrow. After doing this, the total number of packets returned
was "261."
>
